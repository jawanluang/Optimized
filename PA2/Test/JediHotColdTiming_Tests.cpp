...
bool JediConvert_Test = false;

TEST_WITH_TEARDOWN(Jedi_Timing_HotCold, TestConfig::ALL)
{
	...
	CHECK(pFoundColdNode->key == 0x1c663af9);
	...
	Trace::out2("\n");
	Trace::out2(" --- JEDI Problem ------------------\n");
	Trace::out2("\n");
	if (JediConvert_Test == true)
	{
		Trace::out2("  Jedi Bloated List create: %f s\n", t1.TimeInSeconds());
		Trace::out2("      Jedi HotCold convert: %f ms\n", t3.TimeInSeconds() * 1000.0f);
		Trace::out2("\n");
		Trace::out2("         Jedi Bloated find: %f ms\n", t2.TimeInSeconds() * 1000.0f);
		Trace::out2("         Jedi HotCold find: %f ms\n", t4.TimeInSeconds() * 1000.0f);
		Trace::out2("\n");
		Trace::out2("                Jedi Ratio: %f \n", t2.TimeInSeconds() / t4.TimeInSeconds());
		Trace::out2("\n");
	}
	else
	{
		Trace::out2("--- ERROR: Need to pass (JediConvert) tests for timing scores --- \n", t1.TimeInSeconds());
		Trace::out2("\n");
		Trace::out2("  Jedi Bloated List create: %f s\n", t1.TimeInSeconds());
		Trace::out2("      Jedi HotCold convert: %f ms  {INVALID}\n", 999.9f);
		Trace::out2("\n");
		Trace::out2("         Jedi Bloated find: %f ms  {INVALID}\n", 999.9f);
		Trace::out2("         Jedi HotCold find: %f ms  {INVALID}\n", 999.9f);
		Trace::out2("\n");
		Trace::out2("                Jedi Ratio: %f  {INVALID} \n", 0.0f);
		Trace::out2("\n");
	}
...
TEST_TEARDOWN(Jedi_Timing_HotCold)
{
	delete pBloated;
	delete pHotCold;
}
