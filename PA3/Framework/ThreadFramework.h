...
namespace ThreadFramework
{
	...
	public:
		...
		static bool SetName(const std::thread &t0, const char *threadName, int numTabs = 1)
		{
			...
			// register the name
			bool status = pDebug->poDictionary->AddToMap(threadName, threadId, numTabs);
			if (status)
			{
				...
				HRESULT hr = SetThreadDescription(tHandle, wsTmp.c_str());
				AZUL_UNUSED_VAR(hr);
		...
		static bool SetCurrentName(const char * const threadName, int numTabs = 1)
		{
			...
			if (status)
			{
				...
				HRESULT hr = SetThreadDescription(GetCurrentThread(), wsTmp.c_str());
				AZUL_UNUSED_VAR(hr);
				assert(FAILED(hr) == S_OK);
			}
		...
		static void out(const char *const fmt, ...)
		{
			...
			// indent + 1 more 
			size_t numTabs = (size_t)Debug::GetCurrentNumTabs() + 1;
...
namespace ThreadFramework
{
	...
	private:
		void privPrint()
		{
			ThreadID _ThreadID = this->tID;

			size_t numTabs = (size_t) this->tabs;
	...
	public:
			...
			pDict(Debug::GetDictionary())
		{
			...
			if(0x7890abcd == pBannerBase->GetBannerBaseID())
			{
				...
				ThreadID _ThreadID = Debug::GetCurrentID();
				...
				if (pData)
				{
					if (pData->finalized == false)
					{
						...
						bool RemoveStatus = pDict->RemoveFromMap(_ThreadID);
						AZUL_UNUSED_VAR(RemoveStatus);
						assert(RemoveStatus);

						bool status = Debug::SetCurrentName(name,pBannerBase->GetCallingThreadNumTabs()+1);
						this->isThread = status;

						bool FinalizedStatus = pDict->SetFinalized(_ThreadID);
					...
					else
					{
						...
						bool status = Debug::SetCurrentName(pBannerBase->GetBannerName(), pBannerBase->GetCallingThreadNumTabs()+1);
						this->isThread = status;
					}
				}
				else
				...
				size_t numTabs = (size_t)this->pDict->GetNumTabs(_ThreadID);
				...
				else
				{
					...
					int _tabCount = this->pDict->GetNumTabs(_ThreadID) + 1;
		...
		virtual ~Banner()
		{
			...
			if (this->isThread)
			{
				...
				bool status = this->pDict->RemoveFromMap(_ThreadID);
				assert(status);
