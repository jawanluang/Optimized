...
TEST(Mem10_Allocate_3_100_middle_up_free, TestConfig::Flag::ALL)
{
#if Mem10_Allocate_3_100_middle_up_free
...

	{
		...
		CHECK_EQUAL(PTR_FIX(blkStart), PTR_FIX(hdrEnd));
		CHECK_EQUAL(PTR_FIX(blkEnd), 0x8554);
		CHECK_EQUAL(free->mDataSize, 0x4290);
		...
		CHECK_EQUAL(PTR_FIX(blkStart), PTR_FIX(hdrEnd));
		CHECK_EQUAL(PTR_FIX(blkEnd), 0x8554);
		CHECK_EQUAL(free->mDataSize, 0x852C);
