...
TEST(Mem9_Allocate_3_middle_down_free, TestConfig::Flag::ALL)
{
#if Mem9_Allocate_3_middle_down_free
...

	{
		...
		CHECK_EQUAL(free->mDataSize, 0xc1b4);
		...
		CHECK_EQUAL(free->pPrev, nullptr);
		// Next
		CHECK_EQUAL(PTR_FIX(free->pNext), 0x640);
		// Hdr Size
		CHECK_EQUAL(hdrEnd - hdrStart, sizeof(Used));
		...
		CHECK_EQUAL(PTR_FIX(hdrEnd), 0x64c);
		// Prev
		CHECK_EQUAL(PTR_FIX(free->pPrev), 0x228);
		// Next
		CHECK_EQUAL(free->pNext, nullptr);
		// Hdr Size
		CHECK_EQUAL(hdrEnd - hdrStart, sizeof(Free));
